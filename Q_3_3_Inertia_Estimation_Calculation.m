...
accel_range = 2000 * (2 * pi / 60); % Convert 2000 rpm/s to rad/s^2
...
for n = 1:num_phases
    ...
    if any(phase_indices)
        ...
        if a_plus_n ~= a_minus_n
            ...
            J_eq_n(n) = NaN; % Avoid division by zero
        end
    else
...
% Remove NaN values from J_eq_n
J_eq_n = J_eq_n(~isnan(J_eq_n));

% Compute final J_eq by averaging over all valid phases
